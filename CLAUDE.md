...
- **Inyección de dependencias:** Koin `koin-androidx-compose:3.5.3` — **NO usar Hilt ni Dagger**
...
- Inyectados con Koin via `koinViewModel()` — nunca instanciar manualmente en Composables
- **NO usar Hilt.** La inyección es exclusivamente con Koin
...
- **No implementar aún** — es una funcionalidad futura planificada. Si Claude Code detecta un 401 sin manejo, añadir un TODO con el ticket correspondiente
...
1. **Context Leak:** nunca pasar `Activity` o `Context` como parámetro de retención a un ViewModel. Si necesita contexto inicial, pasar `applicationContext` a través del Factory de Koin
...
## 9. Al implementar una nueva feature

1. Crear `data/network/<entidad>/` con los DTOs (Requests + Responses) y la interfaz Retrofit
2. Registrar el servicio en el módulo Koin correspondiente
3. Crear `ui/<feature>/EstadoUiXxx.kt` con el `data class` de estado
4. Crear `ui/<feature>/XxxViewModel.kt` con el `StateFlow` y la lógica
...
7. Añadir los strings nuevos en `res/values/strings.xml`
8. Verificar con `./gradlew test` antes de abrir el PR
9. Rama: `feature/P0-XX-descripcion` — commit con Conventional Commits referenciando la issue
